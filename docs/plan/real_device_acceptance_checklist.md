# 历史真机验收 Checklist（JSON + OSS 目标）

> 状态说明（2026-06-11）：本文件保留为历史验收记录模板。当前项目已调整为 Android-first 性能监测与设备侧报告拉取闭环，新的验证事实源见 `../../TEST.md`，本文件不再定义当前通过标准。

更新时间：2026-03-11（Android 首轮验收已完成）

历史适用范围：`flutter_performance_tier` 2026-03 阶段交付目标（结构化 JSON 产出 + 经业务服务上传 OSS）。
...
- [x] Android 当时阶段通过（结构化 JSON、运行期变化、上传闭环）。
- [ ] iOS 全部通过。
