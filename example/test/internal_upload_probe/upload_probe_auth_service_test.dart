...
import 'package:flutter_performance_tier_example/internal_upload_probe/upload_probe_auth_service.dart';
...
void main() {
  ...
  group('UploadProbeAuthService', () {
    ...
    test(
      'resolveAccessToken logs in and persists session when missing',
      () async {
        final expiresAt = _truncateToSecond(
    ...
    test(
      'resolveAccessToken relogs in when cached session is expired',
      () async {
        ...
        expect(service.currentState.status, AuthStatus.authenticated);
        expect(
          service.currentState.session?.tokens.accessToken,
          refreshedToken,
        );
...
String _buildJwt({required DateTime exp}) {
  ...
  return '${encode(const <String, Object?>{'alg': 'none', 'typ': 'JWT'})}.'
      '${encode(<String, Object?>{'exp': exp.toUtc().millisecondsSinceEpoch ~/ 1000})}.'
      'signature';
}
