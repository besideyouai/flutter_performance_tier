...
- `android/` + `ios/`：plugin 侧平台实现，负责提供 `performance_tier/device_signals` 原生采集能力。
- `example/lib/`：轻量 public example，默认首页只展示可解释的决策摘要、设备信号和策略结果。
- `example/lib/demo/` 内的 `Internal Tools`：仅供联调 / 验收使用，承载 runtime preset、structured logs 和 `/upload probe`。
...
- 推荐从 `flutter run -t example/lib/main.dart` 启动 example
- App 内可一键复制 `AI Diagnostics JSON`
- `flutter test` 会输出 `PERF_TIER_TEST_RESULT` JSON 结果

默认入口现在是 `example/lib/main.dart`，并将 runtime preset / structured logs / upload probe 收纳到 `Internal Tools`。
`flutter run -t example/lib/internal_upload_probe_main.dart` 仍保留，便于把上传链路作为独立入口单独验证。

## 上传探针配置

example 内的 `Internal Tools` 和 `example/lib/internal_upload_probe_main.dart` 按以下优先级读取配置：

1. `--dart-define`
2. `example/lib/internal_upload_probe/internal_upload_probe_env.dart` 对应的 secure env
...
或 `example/lib/internal_upload_probe/internal_upload_probe_env.dart`，
